...
namespace Backend.Fx.EfCorePersistence.Tests
{
    public class DbSession : IDisposable
    {
        ...
        public IUnitOfWork BeginUnitOfWork(bool asReadonly = false, IClock clock = null, IIdentity identity = null)
        {
            ...
            if (asReadonly) transactionContext = new ReadonlyDecorator(transactionContext);
    ...
    public static class TestEx
    {
        ...
        {
            if (unitOfWork is EfUnitOfWork efUnitOfWork) return efUnitOfWork.DbContext;

            if (unitOfWork is DbContextTransactionDecorator dbcTransactionDecorator) return GetDbContext(dbcTransactionDecorator.UnitOfWork);

            if (unitOfWork is DbTransactionDecorator transactionDecorator) return GetDbContext(transactionDecorator.UnitOfWork);

            if (unitOfWork is DbConnectionDecorator connectionDecorator) return GetDbContext(connectionDecorator.UnitOfWork);
        ...
        {
            if (unitOfWork is DbContextTransactionDecorator dbcTransactionDecorator) return (DbTransaction) dbcTransactionDecorator.TransactionContext.CurrentTransaction;

            if (unitOfWork is DbTransactionDecorator transactionDecorator) return (DbTransaction) transactionDecorator.TransactionContext.CurrentTransaction;

            if (unitOfWork is DbConnectionDecorator connectionDecorator) return GetDbTransaction(connectionDecorator.UnitOfWork);
