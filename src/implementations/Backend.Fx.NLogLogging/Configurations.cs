    ...
    public static class Configurations
    {
        ...
        public static void ForTests(string appRootNamespace, string logfilename = "tests.xlog")
        {
            lock (SyncLock)
            {
                ...
                {
                    Layout
                        = @"${level:uppercase=true:padding=5} ${mdc:item=Activity} ${time} ${logger} ${message} ${exception}"
                };
                ...
                config.LoggingRules.Add(
                    new LoggingRule("Microsoft.AspNetCore.Hosting.Internal.WebHost", LogLevel.Info, fileTarget));
                ...
                Console.WriteLine(
                    $"Test console shows only warn, error and fatal events. Full log file is available at {Path.Combine(AppContext.BaseDirectory, logfilename)}");
            }
        }
