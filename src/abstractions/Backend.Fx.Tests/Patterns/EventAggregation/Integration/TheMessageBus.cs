...
namespace Backend.Fx.Tests.Patterns.EventAggregation.Integration
{
    ...
    public abstract class TheMessageBus
    {
        ...
        protected TestInvoker Invoker { get; } = new();
        ...
        {
            public readonly TestIntegrationEvent IntegrationEvent = new(34, "gaga");

            public TestInvoker()
            {
                A.CallTo(() => TypedHandler.Handle(A<TestIntegrationEvent>._))
                    .Invokes((TestIntegrationEvent _) => IntegrationEvent.TypedProcessed.Set());
                A.CallTo(() => DynamicHandler.Handle(new object()))
                    .WithAnyArguments()
                    .Invokes((object _) => IntegrationEvent.DynamicProcessed.Set());
