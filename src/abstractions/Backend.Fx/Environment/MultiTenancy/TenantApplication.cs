...
namespace Backend.Fx.Environment.MultiTenancy
{
    public abstract class TenantApplication
    {
        ...
        {
            _application.MessageBus.Subscribe(
                new DelegateIntegrationMessageHandler<TenantActivated>(
                    tenantCreated =>
                    {
                        Logger.Info(
                            $"Seeding data for recently activated {(tenantCreated.IsDemoTenant ? "demo " : "")}tenant {tenantCreated.TenantId}");
                        try
                        {
                            _dataGenerationContext.SeedDataForTenant(
                                new TenantId(tenantCreated.TenantId),
                        ...
                        catch (Exception ex)
                        {
                            Logger.Error(
                                ex,
                                $"Seeding data for recently activated {(tenantCreated.IsDemoTenant ? "demo " : "")}tenant {tenantCreated.TenantId} failed.");
                        }
                    }));
        }
    }
