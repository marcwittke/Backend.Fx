...
namespace Backend.Fx.Environment.MultiTenancy
{
    ...
    public interface ITenantService
    {
        ...
        /// be implemented by a client to the master tenant service, probably using a remoting technology like RESTful Service,
        /// HTTP,
    ...
    public class TenantService : ITenantService
    {
        ...
        {
            Tenant[] tenants = _tenantRepository.GetTenants();
        ...
        {
            Tenant[] activeTenants = _tenantRepository
                .GetTenants()
        ...
        {
            Tenant[] activeDemonstrationTenants = _tenantRepository
                .GetTenants()
                .Where(t => t.State == TenantState.Active && t.IsDemoTenant)
                .ToArray();
            Logger.Trace(
                $"Active Demonstration TenantIds: {string.Join(",", activeDemonstrationTenants.Select(t => t.ToString()))}");
            return activeDemonstrationTenants;
        }

        public Tenant[] GetActiveProductionTenants()
        {
            Tenant[] activeProductionTenants = _tenantRepository
                .GetTenants()
                .Where(t => t.State == TenantState.Active && !t.IsDemoTenant)
                .ToArray();
            Logger.Trace(
                $"Active Production TenantIds: {string.Join(",", activeProductionTenants.Select(t => t.ToString()))}");
